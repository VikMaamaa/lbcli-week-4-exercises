# Create a raw transaction and add this message in it: "btrust builder 2026"

# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
# transaction=""
#!/bin/bash

# Source raw transaction (contains the UTXOs i want to spend)
source_tx_hex=""

# Message to embed in the blockchain via OP_RETURN
embedded_message="btrust builder 2026"

# Converting message to hex format (required for OP_RETURN output)
op_return_hex=$(printf '%s' "$embedded_message" | xxd -p | tr -d '\n')

# Decoding transaction to get TXID (transaction hash)
decoded_txid=$(bitcoin-cli -regtest -named decoderawtransaction hexstring=$source_tx_hex | jq -r '.txid')

# Extract the output indexes (vout) of UTXOs to be used as inputs
first_output_index=$(bitcoin-cli -regtest -named decoderawtransaction hexstring=$source_tx_hex | jq -r '.vout[0].n')
second_output_index=$(bitcoin-cli -regtest -named decoderawtransaction hexstring=$source_tx_hex | jq -r '.vout[1].n')

# Destination address to receive 0.2 BTC (20,000,000 satoshis)
destination_address="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"

# Create raw transaction:
# - Uses two UTXOs as inputs
# - Sends 0.2 BTC to recipient
# - Embeds message using OP_RETURN (data field)
created_raw_tx=$(bitcoin-cli -regtest -named createrawtransaction \
  inputs='''[
    { "txid": "'$decoded_txid'", "vout": '$first_output_index' },
    { "txid": "'$decoded_txid'", "vout": '$second_output_index' }
  ]''' \
  outputs='''{
    "data": "'$op_return_hex'",
    "'$destination_address'": 0.20000000
  }''')

# Output the final raw transaction hex (unsigned)
echo $created_raw_tx